...
static const int FIRRTL_MAX_DSH_WIDTH_ERROR = 20; // For historic reasons, this is actually one greater than the maximum
						  // allowed shift width
...
std::string dump_const(const RTLIL::Const &data)
{
	...
	if (data.flags & RTLIL::CONST_FLAG_REAL) {
		// Yosys stores real values as strings, so we call the string dumping
		// code.
	...
	// Numeric (non-real) parameter.
		...
		if (width <= 32) {
			...
			// If value is larger than 32 bits, then emit a binary
			// representation of the number as integers are not large enough to
			// contain the result. There is a caveat to this approach though:
			//
			// Note that parameter may be defined as having a fixed width as
			// follows:
			//
			...
			// assigns it a width of 32 bits regardless of the type of the
			// parameter.
			//
			//     defparam <inst_name> .test_signed = 49;           (width =
			//     32, though should be 27 based on definition) defparam
			//     <inst_name> .test_unsigned = 40'd35;     (width = 40, though
			//     should be 27 based on definition) defparam <inst_name>
			//     .test_signed_large = 40'd12; (width = 40)
			//
			// We therefore may lose the precision of the original verilog
			// literal if it was written without its bitwidth specifier.
...
void emit_extmodule(RTLIL::Cell *cell, RTLIL::Module *mod_instance, std::ostream &f)
{
	...
	for (auto wire : mod_instance->wires()) {
		...
		const std::string portDecl = stringf("%s%s %s: UInt<%d> %s\n", indent.c_str(), wire->port_input ? "input" : "output", wireName,
						     wire->width, wireFileinfo.c_str());
	...
	for (const auto &p : cell->parameters) {
		...
		// Remove backslashes from parameters as these come from the internal
		// RTLIL naming scheme, but should not exist in the emitted firrtl
		// blackboxes. When firrtl is converted to verilog and given to
		// downstream synthesis tools, these tools expect to find blackbox names
		// and parameters as they were originally defined, i.e. without the
		// extra RTLIL naming conventions.
		param_name.erase(std::remove(param_name.begin(), param_name.end(), '\\'), param_name.end());
...
struct FirrtlWorker {
		...
		for (auto chunk : sig.chunks()) {
			...
			if (chunk.wire == nullptr) {
				...
				for (int i = GetSize(bits) - 4; i >= 0; i -= 4) {
					int val = 0;
					if (bits[i + 0] == State::S1)
						val += 1;
					if (bits[i + 1] == State::S1)
						val += 2;
					if (bits[i + 2] == State::S1)
						val += 4;
					if (bits[i + 3] == State::S1)
						val += 8;
	...
	std::string fid(RTLIL::IdString internal_id) { return make_id(internal_id); }

	std::string cellname(RTLIL::Cell *cell) { return fid(cell->name).c_str(); }

	void process_instance(RTLIL::Cell *cell, vector<string> &wire_exprs)
	{
		std::string cell_type = fid(cell->type);
		std::string instanceOf;
		// If this is a parameterized module, its parent module is encoded in
		// the cell type
		...
		// If the instance is that of a blackbox, use the modified extmodule
		// name that contains per-instance parameterizations. These instances
		// were emitted earlier in the firrtl backend.
		const std::string instanceName = instModule->get_blackbox_attribute() ? extmodule_name(cell, instModule) : instanceOf;

		std::string cellFileinfo = getFileinfo(cell);
		wire_exprs.push_back(stringf("%s"
					     "inst %s%s of %s %s",
					     indent.c_str(), cell_name.c_str(), cell_name_comment.c_str(), instanceName.c_str(),
					     cellFileinfo.c_str()));
		...
		for (auto it = cell->connections().begin(); it != cell->connections().end(); ++it) {
			if (it->second.size() > 0) {
				...
				switch (dir) {
				case FD_INOUT:
					log_warning("Instance port connection %s.%s is INOUT; "
						    "treating as OUT\n",
				...
				case FD_OUT:
					...
					break;
				case FD_NODIRECTION:
					log_warning("Instance port connection %s.%s is "
						    "NODIRECTION; treating as IN\n",
						    cell_type.c_str(), log_signal(it->second));
					YS_FALLTHROUGH
				case FD_IN:
					...
					break;
				default:
					log_error("Instance port %s.%s unrecognized connection "
						  "direction 0x%x !\n",
				...
				if (sinkExpr.compare(0, bitsString.length(), bitsString) == 0) {
					...
					// Add the source and sink to the "reverse_wire_map" and
					// we'll output the assignment
					//  as part of the coalesced subfield assignments for this
					//  wire.
					register_reverse_wire_map(sourceExpr, *sinkSig);
				} else {
					wire_exprs.push_back(
	...
	//  generate the FIRRTL expression for equivalent dynamic shift taking into
	//  account FIRRTL shift semantics.
	std::string gen_dshl(const string b_expr, const int b_width)
	{
		...
		if (b_width >= FIRRTL_MAX_DSH_WIDTH_ERROR) {
			...
			result = stringf("mux(gt(%s, %s), %s, bits(%s, %d, 0))", b_expr.c_str(), max_shift_string.c_str(), max_shift_string.c_str(),
					 b_expr.c_str(), max_shift_width_bits - 1);
		}
		return result;
	}
	...
	void emit_module()
	{
		...
		for (auto wire : module->wires()) {
			...
			// If a wire has initial data, issue a warning since FIRRTL doesn't
		...
		for (auto cell : module->cells()) {
			...
			bool firrtl_is_signed = a_signed; // The result is signed (subsequent code may change
							  // this).
			int firrtl_width = 0;
			string primop;
			...
			if (cell->type.in(ID($not), ID($logic_not), ID($_NOT_), ID($neg), ID($reduce_and), ID($reduce_or), ID($reduce_xor),
				...
				// Don't use the results of logical operations (a single bit) to
				// control padding
				if (!(cell->type.in(ID($eq), ID($eqx), ID($gt), ID($ge), ID($lt), ID($le), ID($ne), ID($nex), ID($reduce_bool),
						    ID($logic_not)) &&
				      y_width == 1)) {
				...
				if (cell->type.in(ID($not), ID($_NOT_)))
					...
					primop = "neq";
					// Use the sign of the a_expr and its width as the type
					// (UInt/SInt) and width of the comparand.
			...
			if (cell->type.in(ID($add), ID($sub), ID($mul), ID($div), ID($mod), ID($xor), ID($_XOR_), ID($xnor), ID($and), ID($_AND_),
					  ID($or), ID($_OR_), ID($eq), ID($eqx), ID($gt), ID($ge), ID($lt), ID($le), ID($ne), ID($nex), ID($shr),
					  ID($sshr), ID($sshl), ID($shl), ID($logic_and), ID($logic_or), ID($pow))) {
				string a_expr = make_expr(cell->getPort(ID::A));
				...
				// Shift amount is always unsigned, and needn't be padded to
				// result width,
				...
				// For the arithmetic ops, expand operand widths to result
				// widths befor performing the operation. This corresponds
				// (according to iverilog) to what verilog compilers implement.
				if (cell->type.in(ID($add), ID($sub), ID($mul), ID($div), ID($mod), ID($xor), ID($_XOR_), ID($xnor), ID($and),
						  ID($_AND_), ID($or), ID($_OR_))) {
					if (a_width < y_width) {
						a_expr = stringf("pad(%s, %d)", a_expr.c_str(), y_width);
						a_width = y_width;
					}
				...
				if (cell->type == ID($add)) {
					...
					// FIRRTL will widen the result (y) by the amount of the
					// shift. We'll need to offset this by extracting the
					// un-widened portion as Verilog would do.
					extract_y_bits = true;
					...
					// We'll need to do some special fixups if the source (and
					// thus result) is signed.
					if (firrtl_is_signed) {
						// If this is a "logical" shift right, pretend the
						// source is unsigned.
						if (cell->type == ID($shr)) {
					...
					if (a_sig.is_fully_const() && a_sig.as_int() == 2) {
						// We'll convert this to a shift. To simplify things,
						// change the a_expr to "1"
				...
				// Deal with FIRRTL's "shift widens" semantics, or the need to
				// widen the FIRRTL result. If the operation is signed, the
				// FIRRTL width will be 1 one bit larger.
				if (extract_y_bits) {
			...
			if (cell->type == ID($shiftx)) {
				...
				if (cell->getParam(ID::B_SIGNED).as_bool()) {
					// Use validif to constrain the selection (test the sign
					// bit)
			...
			if (cell->type == ID($pos)) {
				...
				//				printCell(cell);
				string a_expr = make_expr(cell->getPort(ID::A));
				// Verilog appears to treat the result as signed, so if the
				// result is wider than "A",
				//  we need to pad.
				if (a_width < y_width) {
					a_expr = stringf("pad(%s, %d)", a_expr.c_str(), y_width);
				}
			...
			for (int i = 0; i < GetSize(mem.wr_ports); i++) {
				...
				for (int i = 1; i < GetSize(port.en); i++)
					if (port.en[0] != port.en[i])
						log_error("Complex write enable on port %d on memory "
							  "%s.%s.\n",
							  i, log_id(module), log_id(mem.memid));
		...
		for (auto wire : module->wires()) {
			...
			while (cursor < wire->width) {
				...
				if (reverse_wire_map.count(start_bit)) {
					...
					new_expr =
					  stringf("bits(%s, %d, %d)", start_map.first.c_str(), start_map.second + chunk_width - 1, start_map.second);
					is_valid = true;
				} else {
					if (unconn_id.empty()) {
						unconn_id = next_id();
			...
			if (is_valid) {
				if (make_unconn_id) {
					wire_decls.push_back(
					  stringf("%swire %s: UInt<1> %s\n", indent.c_str(), unconn_id.c_str(), wireFileinfo.c_str()));
					// `invalid` is a firrtl construction for simulation so we
					// will not tag it with a @[fileinfo] tag as it doesn't
					// directly correspond to a specific line of verilog code.
				...
				// `invalid` is a firrtl construction for simulation so we will
				// not tag it with a @[fileinfo] tag as it doesn't directly
				// correspond to a specific line of verilog code.
...
struct FirrtlBackend : public Backend {
	...
	void execute(std::ostream *&f, std::string filename, std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		// If we weren't explicitly passed a filename, use the last argument (if
		// it isn't a flag).
		if (filename == "") {
		...
		// Get the top module, or a reasonable facsimile - we need something for
		// the circuit name.
